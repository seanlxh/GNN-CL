...
class SageConv(Module):
    ...
    def forward(self, features, adj):
        ...
        if not isinstance(adj.cpu(), torch.sparse.FloatTensor):
            if len(adj.shape) == 3:
...
class MLP_En2(nn.Module):
    def __init__(self, nfeat, nhid, nembed, dropout):
        super(MLP_En2, self).__init__()

        self.mlp1 = nn.Linear(nfeat, nhid)
        self.mlp2 = nn.Linear(nhid, nembed)
        self.dropout = dropout
...
class SmoothDecoder(Module):
    ...
    def __init__(self, nembed, dropout=0.1):
        ...
        self.de_weight_p = Parameter(torch.FloatTensor(nembed, nembed))
        self.de_weight_q = Parameter(torch.FloatTensor(nembed, nembed))

        self.reset_parameters()

    def reset_parameters(self):
        stdvp = 1. / math.sqrt(self.de_weight_p.size(1))
        stdvq = 1. / math.sqrt(self.de_weight_q.size(1))

        self.de_weight_p.data.uniform_(-stdvp, stdvp)
        self.de_weight_q.data.uniform_(-stdvq, stdvq)

    def forward(self, node_embed):
        combine1 = F.linear(node_embed, self.de_weight_p)
        combine2 = F.linear(node_embed, self.de_weight_q)

        adj_out = torch.sigmoid(torch.mm(combine1, combine2.transpose(-1, -2)))
...
#
#     def forward(self, node_embed):
#         P = F.linear(node_embed, self.de_weight_p)
#         Q = F.linear(node_embed, self.de_weight_q)
#         Q = torch.mm(P, Q.transpose(-1, -2))
#         P = torch.mul(P, P)
#         P = torch.sum(P, dim=1)
#         P = P.expand(P.shape[0], P.shape[0])
#         P=P.T
#         combine = P-Q  #调参 tanh
#         adj_out = torch.sigmoid(combine)
#         # adj_out = torch.softmax(combine, dim=1)
#         return adj_out
#
